...
│   └── test/                      # 101 offline tests (domain, repository, provider, widget_flow,
...
│   │   └── test/index.test.ts     # 54 functions tests (firebase-functions-test + emulator)
│   └── rules-tests/test/firestore.test.js  # 39 rules tests (@firebase/rules-unit-testing)
│
└── docx/                          # documentation
...
| **M13** | **App menu** — centralized the AppBar in the shell with a navigation **Drawer** (`presentation/features/menu/`): Contact us, About (built-in `showAboutDialog`), Rate us on Google Play, Terms, Privacy, Sign out + a dismiss button. | `master_m13` |
...
flutter test           # 101 tests: domain, repository, provider, widget_flow, error_mapper, accessibility
