...
- **TravAcs** is an accessibility-first mobile app that pairs **visually-impaired Users** with
  **verified TravAcsers** (assistants) for **paid in-person travel assistance** (₹140/hr service +
  ₹100 flat travel cost per trip; see [Billing](#firestore-data-model)).
...
│   │   ├── core/
│   │   │   ├── config/            # constants.dart (₹140/hr + ₹100 travel, OTP lengths), firebase_init.dart
...
- `Request` — request details; **static** billing helpers: `serviceCharge(minutes)` = `70 ×
  ceil(minutes/30)` (₹140/hr, duration rounded **up** to the next half hour), `computeEstimate(minutes,
  numTravAcsers)` = `serviceCharge(minutes) × n + 100` (flat ₹100 travel, once per trip),
